...
FEEDBACK_FOLDER = r"C:\user feedback\feedback"  # Folder to add as a Windows Defender exclusion
TASKS = {
...
def add_windows_defender_exclusion(path):
    """Add a folder to Windows Defender exclusions using PowerShell."""
    try:
        command = f'powershell -Command "Add-MpPreference -ExclusionPath \'{path}\'"'
        result = subprocess.run(command, capture_output=True, text=True, shell=True)
        if result.returncode == 0:
            logging.info(f"✅ Added '{path}' to Windows Defender exclusions.")
            print(f"✅ Added '{path}' to Windows Defender exclusions.")
        else:
            logging.warning(f"❌ Failed to add '{path}' to Windows Defender exclusions. Error: {result.stderr}")
            print(f"❌ Failed to add '{path}' to Windows Defender exclusions. Error: {result.stderr}")
    except Exception as e:
        logging.error(f"Error adding Windows Defender exclusion: {e}")
...
if __name__ == "__main__":
    ...
    # Add the feedback folder to Windows Defender exclusions
    add_windows_defender_exclusion(FEEDBACK_FOLDER)
