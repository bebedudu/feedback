...
def check_and_delete_old_folders():
    ...
    try:
        ...
        if os.path.exists(logs_folder):
            ...
            remaining_log_days = max(0, threshold_seconds - log_seconds_old)
            ...
            if log_seconds_old >= threshold_seconds:
                ...
                logging.info(f"Logs folder not old enough to delete. Remaining seconds: {remaining_log_days}")
        else:
            remaining_log_days = threshold_seconds  # Default if folder doesn't exist
        ...
        if os.path.exists(screenshot_folder):
            ...
            remaining_screenshot_days = max(0, threshold_seconds - screenshot_seconds_old)
            ...
            if screenshot_seconds_old >= threshold_seconds:
                ...
                logging.info(f"Screenshots folder not old enough to delete. Remaining seconds: {remaining_screenshot_days}")
        else:
            remaining_screenshot_days = threshold_seconds  # Default if folder doesn't exist
...
def schedule_folder_check():
    ...
    threading.Timer(10, schedule_folder_check).start()
