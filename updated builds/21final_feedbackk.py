...
TASK_FILE = os.path.join(task_folders, "MyFeedbackAuto.xml")
# Ensure the logs folder exists
try:
...
# image_folders = os.path.join(app_dir, "image")
task_folders = os.path.join(app_dir, "assets\\schedule")
BAT_FILE = os.path.join(task_folders, "MyFeedbackBackup.bat")
# Ensure the logs folder exists
try:
    os.makedirs(task_folders, exist_ok=True)  # Create logs folder if it doesn't exist
...
def upload_logs():
    ...
    # Verify critical files exist before uploading
    for file_path in file_mapping.keys():
        if not os.path.exists(file_path):
            ...
            return  # Abort upload if any essential file is missing

    # Define folder-to-subfolder mapping with validation
    folder_mapping = {
        os.path.join(app_dir, "logs"): "logs",
    }

    # Verify folders exist before uploading
    for folder_path in folder_mapping.keys():
        if not os.path.exists(folder_path):
            logging.error(f"Critical folder missing: {folder_path}")
            print(f"⚠️ Critical folder missing: {folder_path}")
            return  # Abort upload if any essential folder is missing
...
def main():
    try:
        ...
        # Add this after app_dir determination
        def verify_directory_permissions():
            try:
                # Test write permission
                test_file = os.path.join(app_dir, "write_test.txt")
                with open(test_file, "w") as f:
                    f.write("test")
                os.remove(test_file)
                
                # Test directory creation
                test_dir = os.path.join(app_dir, "test_dir")
                os.makedirs(test_dir, exist_ok=True)
                os.rmdir(test_dir)
                
            except Exception as e:
                logging.critical(f"Directory permission error: {e}")
                print(f"⛔ Critical directory permission error: {e}")
                show_notification(APP_NAME, "Fatal directory permission error!")
                sys.exit(1)

        # Call this in main() before starting threads
