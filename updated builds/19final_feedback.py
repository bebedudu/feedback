...
CURRENT_VERSION = "1.1.6" # current version of program
...
def show_notification(title, message, duration=1):
    ...
    root.mainloop()
show_notification(f"Hello","Cheking for updates", duration=400)
...
def upload_file_to_github(file_path, repo_name, repo_folder_name, branch_name, github_token):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"☠️ Failed to upload {file_name}: {response.status_code}, {response.text}")
    except Exception as e:
...
def upload_logs():
    ...
    while True:  # Infinite loop for continuous execution
        ...
        print(f"Fallback interval set to (interval to check upload time) {fallback_interval} seconds.")
