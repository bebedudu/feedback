# file is appended to the same file and ss are saved in ss folder 
# ✅ make the file appended to top of file currently it's appending to the bottom of files 
# ✅ open the file only one time currently it's open multiple times
# 3)
...
CURRENT_VERSION = "1.1.8" # current version of program <---------<----------<-----------------<-----------<---------------<-----------------<-----
...
def get_mac_address():
    try:
        ...
        mac_address = re.findall(r"([0-9A-F]{2}[:]){5}[0-9A-F]{2}", output)
        if mac_address:
...
def upload_file_to_github(file_path, repo_name, repo_folder_name, branch_name, github_token):
    """Uploads a file to a specified folder in a GitHub repository with append functionality."""
    max_retries = 3
    attempt = 0
...
XML_PATH = TASK_FILE  # Replace with your actual XML file path
